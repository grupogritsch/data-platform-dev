#!/usr/bin/env python3
"""
Aplicação da Regra Oficial de Identificação de Provedores na View do DW:
- OMNILINK: complemento contém 'OMNILINK'
- NUXEO: complemento contém 'GRIT' ou 'ST8310'
- 3STEC: registros da 3STEC
"""
import os
import psycopg2
...
-- ============================================================================
-- 1. VIEW UNIFICADA E SANEADA DE ALERTAS DE TELEMETRIA (NUXEO + OMNILINK + 3STEC)
-- ============================================================================
...
cur.execute(sql_view_provedores)
conn.commit()
print("✅ View atualizada com a lógica exata de provedores (OMNILINK, NUXEO, 3STEC)!")

print("\n--- DISTRIBUIÇÃO DOS ÚLTIMOS 14 DIAS POR PROVEDOR REAL ---")
cur.execute("""
    SELECT 
        provedor_rastreador,
...
for r in cur.fetchall():
    print(f"  📡 Provedor: {r[0]:10} -> {r[1]:4} alertas | {r[2]:3} veículos | Pico: {r[3]:.0f} km/h | Média: {r[4]} km/h")
