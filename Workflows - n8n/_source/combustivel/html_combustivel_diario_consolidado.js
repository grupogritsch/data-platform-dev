// ===========================================================================
// Consolidar Geral (Diretoria) — Combustível DIÁRIO
// Mesma logica do consolidado semanal, mas so a tabela de KPIs
// (empresa toda) e o ranking por filial -- sem Top10 veiculos, sem postos.
// Recipient logic identica: destinatarios/CC de torre.email_gritsch_config.
// runOnceForAllItems (default).
// ===========================================================================
...
const topPostos = cfgNode.top_postos || 8;
const dataRef = perNode.dataRef || 'Hoje';
...
for (const item of items) {
  ...
  const kmFilial = parseFloat(d.total_km_calculado || 0);
  const kmFilialAnt = parseFloat(d.total_km_ant || 0);
  const custoKmFilial = parseFloat(d.custo_km_comb_medido || 0);
  ...
  filiaisList.push({ nome: nomeFilial, gastoComb, litros, veic, custoKmFilial });
  ...
  for (const p of postos) {
    ...
    if (!postosMap[pKey]) {
      postosMap[pKey] = { posto: p.posto || 'Posto', cidade: p.cidade || 'N/D', uf: p.uf || 'N/D',
        combustivel: p.combustivel || 'Diesel', litros: 0, gasto: 0, preco_anp: parseFloat(p.preco_anp || 6.22) };
    }
...
// Ranking por filial -- so as que abasteceram no dia (a SQL ja filtra
// total_litros > 0), ordenado por maior gasto.
const filiaisOrdenadas = filiaisList.slice().sort((a, b) => b.gastoComb - a.gastoComb);
let rowsFiliais = '';
for (const f of filiaisOrdenadas) {
  rowsFiliais += '<tr>'
    ...
    + '<td style="padding:9px 16px;text-align:right;color:' + CINZA + ';font-size:12px;border-bottom:1px solid ' + LINHA + ';font-variant-numeric:tabular-nums">' + numBr(f.litros, 0) + ' L</td>'
    ...
    + '<td style="padding:9px 16px;text-align:right;color:' + CINZA + ';font-size:12px;border-bottom:1px solid ' + LINHA + ';font-variant-numeric:tabular-nums">' + (f.custoKmFilial ? 'R$ ' + fmtDec(f.custoKmFilial, 2) + '/km' : 'N/D') + '</td>'
    + '</tr>';
}
if (filiaisOrdenadas.length > 0) {
  rowsFiliais += linhaTotal([
    'TOTAL GERAL', String(sumVeiculos), numBr(sumLitros, 0) + ' L', moeda(sumGastoComb),
    custoKmCombGeral ? 'R$ ' + fmtDec(custoKmCombGeral, 2) + '/km' : 'N/D',
  ]);
}

// Postos parceiros hoje -- rede toda, com corte Top N (diferente do
// ranking por filial: numero de postos distintos pode crescer rapido com
// 27 filiais somadas, entao aqui SIM cabe um corte, igual ao semanal).
const topPostosLista = Object.values(postosMap).map(p => {
...
}).sort((a, b) => b.gasto - a.gasto).slice(0, topPostos);

let rowsPostos = '';
for (const p of topPostosLista) {
  const desvioStr = (p.desvio_pct > 0 ? '+' : '') + fmtDec(p.desvio_pct, 1) + '%';
  const desvioCor = p.desvio_pct <= 0 ? VERDE_ESC : '#C23B33';
  rowsPostos += '<tr>'
...
+ '<div style="font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:' + CINZA + ';margin-top:10px;font-family:' + F_CORPO + '">Torre de Controle · Combustível diário · visão consolidada</div>'
+ '<div style="font-size:19px;font-weight:800;color:' + GRAFITE + ';margin-top:3px;font-family:' + F_DISPLAY + '">Diretoria · todas as filiais</div>'
+ '</td>'
...
+ (filiaisOrdenadas.length > 0 ? (
  ...
  + '<tr>' + th('Filial') + th('Veíc.', 'center') + th('Litros', 'right') + th('Gasto', 'right') + th('Custo/km', 'right') + '</tr>'
  + rowsFiliais + '</table></td></tr>'
) : (
  '<tr><td style="padding:22px 28px 0">'
  + '<div style="background:' + FUNDO + ';border:1px solid ' + LINHA + ';border-radius:6px;padding:14px 18px;font-size:12.5px;color:' + CINZA + ';font-family:' + F_CORPO + '">Nenhuma filial registrou abastecimento no período.</div>'
  + '</td></tr>'
))

+ (topPostosLista.length > 0 ? (
  faixaSecao('Postos parceiros hoje · Top ' + topPostos)
  + '<tr><td style="padding:0 28px"><table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ' + LINHA + ';border-top:none;border-radius:0 0 6px 6px">'
...
+ '<div style="font-size:11.5px;color:' + CINZA + ';font-family:' + F_CORPO + '">Ranking por placa e comparativo por categoria: relatório semanal, com CSVs em anexo.</div>'
+ '</td></tr>'
...
// Recipient logic: DB-driven, mesmo padrao do semanal/telemetria.
